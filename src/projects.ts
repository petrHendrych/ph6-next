...
/**
 * Projects with a detail page — one entry per directory under
 * `public/projects/`. The `[slug]` route and `subpageTitles` derive from it.
 *
 * **Nothing under `'use client'` may import this file.** Turbopack bundles an
 * imported module whole, so reaching it from the client would ship all 54
 * projects (~60 kB) to the landing page, which renders none of them.
 *
 * TODO: `year`, `Rozsah` and `paragraphs` are placeholders everywhere,
 * `location` only where the title names a place, and the alt texts are indexed
 * rather than descriptive. Fill in before launch.
 */
...
/** Titles for `SubpageHeader`. Natural case — the header uppercases them. */
