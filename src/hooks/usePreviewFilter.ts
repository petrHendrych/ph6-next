...
export const usePreviewFilter = (options: Options = {}) => {
	...
	const runTransition = (
		...
		mm.add(
			...
			context => {
				const isReduced = context.conditions?.reduce;
				const duration = isReduced ? 0 : totalDuration;
				...
				const toShow = afterVisibleEls.filter(el =>
					currentlyHidden.includes(el)
				);
				if (toShow.length) {
				...
				cleanupCallRef.current = gsap.delayedCall(duration, () => {
